...
`tool_result.v1` は、package、client、golden fixture が依存した時点で public implementation contract として扱います。
...
これらの preference は、Secretary が自分の仕事場に対して持つ professional judgment を表します。client、package、支援エージェントは変更を提案できます。永続 preference は Secretary が所有します。
