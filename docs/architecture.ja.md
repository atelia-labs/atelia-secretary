...
Atelia Secretary は、Atelia の backend daemon です。クライアント、AEP package、Surface Protocol、Hook の規範的仕様は [`atelia`](https://github.com/atelia-labs/atelia/blob/main/README.ja.md) リポジトリで扱い、この文書では daemon 実装の境界だけを扱います。

AEP において、Secretary は reference backend host です。backend runtime boundary、permission / capability enforcement、service brokering、Hook 実行境界、audit、registry / blocklist enforcement、install record、quarantine、revocation、rollback を実装します。
...
- `atelia-extensions`: 現在の beta crate。AEP backend package manifest、host runtime、capability boundary を扱う
...
この surface の durable implementation target は [Secretary Runtime Architecture](runtime-architecture.ja.md) で定義します。
...
Atelia Secretary は、Atelia の project-level 仕様に従って AEP package、service broker、Hook の実行境界を実装します。

daemon 側の責務:

- manifest、source policy、registry metadata、compatibility contract の検証
- package / Hook の実行権限チェック
- policy に基づく許可、拒否、承認要求
- audit log の記録
- repository、secret、package、brokered service、外部サービスへのアクセス境界
- 危険な実行経路の block

Secretary は raw GitHub publication を gate しません。ユーザーは Atelia の外で repository、fork、branch、release、pull request を作れます。Secretary が gate するのは、Atelia が resolve / search / install / mount / quarantine / revoke / execute できるかであり、registry submission、source-policy check、manifest validation、permission analysis、service broker policy、audit を通します。
...
規範的な AEP package、Surface Protocol、registry、service、broker、Hook 仕様は Atelia 本体の文書を参照します。
