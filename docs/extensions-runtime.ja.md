...
Observational Memory はこの lane の official default `memory_strategy`
foundation です。recent な raw `messages` を bounded な context buffer として
扱い、より長く残すべき context は `memory` に圧縮します。これにより、
short-term の会話状態と durable な memory を分けて保てます。
