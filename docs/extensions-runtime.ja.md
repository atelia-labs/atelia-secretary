...
Secretary は現在 backend-only compatibility slice を validate します。public AEP package schema は `aep.package.v0` です。この daemon slice は実装が追いつくまで `atelia.extension.v1` です。ただし product / docs language では generic extension ではなく AEP package として扱います。
