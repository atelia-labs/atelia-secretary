...
Observational Memory はこの lane の official default `memory_strategy` として位置づけられ、
当該 contract は将来の package 実行のために予約されます。
`messages` と `memory` の分割を schema / policy coverage として定義しますが、
Secretary はまだその compaction flow を実行しません。
