...
現在の beta 実装は、restart 可能な Secretary state を
`<ATELIA_DAEMON_STORAGE_DIR>/ledger.json` の atomic JSON snapshot として
保存します。各 ledger mutation は validation 後にこの snapshot を再書き込み
します。

startup の挙動は厳格です。

- malformed JSON は startup failure になります
- unsupported snapshot schema version は startup failure になります
- referentially inconsistent records は startup failure になります
- daemon は corrupted durable state を推測や auto-heal しません
...
最初の lifecycle boundary では、`job: queued` と initial `job_event` を1つの atomic commit で永続化します。`policy_decision` は、どの `tool_invocation`、`tool_result`、audit effect record より先に durable commit される必要があります。成功した `SubmitJob` の idempotency record も snapshot に request signature と replayable receipt として保存します。beta JSON snapshot loader は startup 時にこれらの参照を validate し、不完全または不整合な workflow を含む snapshot では fail closed します。automatic policy re-evaluation と tool retry / cleanup reconciliation は、将来の WAL または database-backed store に reserved されています。

`new()` が使う beta の in-memory state は process-local だけに限られます。daemon の lifetime に bound され、restart 後に reset されます。
`new_durable()` が使う beta state は `<ATELIA_DAEMON_STORAGE_DIR>/ledger.json` の atomic JSON snapshot を startup 時に validate したうえで restart 時に restore されます。Health はこの扱いを hint として公開し、beta state を durable storage と誤解させないようにします。
