...
beta の in-memory state は process-local だけに限られます。daemon の lifetime に bound され、restart 後に ledger からは recovery されず、daemon process の再起動で reset されます。Health はこの扱いを hint として公開し、beta state を durable storage と誤解させないようにします。
