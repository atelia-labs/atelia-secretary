...
この table は required service contract であり、現在の protobuf implementation の説明ではありません。現在の daemon は health、repository、job、policy、tool-output settings、`RenderToolOutput` group をすでに expose しています。`WatchEvents` と `GetProjectStatus` はまだ planned contract surface です。
