...
この table は required service contract であり、現在の protobuf implementation だけの説明ではありません。現在の daemon は health、repository、job、policy、event replay、project status、tool-output settings、`RenderToolOutput`、extension RPC group をすでに expose しています。extension install / blocklist operation は現時点では daemon HTTP/JSON transport で expose され、protobuf service は shared client runtime contract に集中しています。`WatchEvents` は現時点では cursor replay surface として実装されており、long-lived streaming transport は future transport refinement です。
