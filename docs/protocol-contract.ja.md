...
`ListRepertoire` は [Agent Repertoire](https://github.com/atelia-labs/atelia/blob/main/docs/agent-repertoire.ja.md) の最初の beta protocol slice として予定されています。persisted store ではなく computed projection を返します。

beta slice では extension install、status、blocklist の API は
operator-facing な management API で、`capability.discovery` 経由で見つけられる
想定です。extension execution の RPC は意図的に用意せず、client が execution RPC
を probe した場合は unsupported-capability failure として扱うべきです。
## Core Messages
