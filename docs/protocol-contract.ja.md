...
この table は required service contract であり、現在の protobuf implementation だけの説明ではありません。現在の daemon は health、repository、job、policy、event replay、project status、tool-output settings、`RenderToolOutput`、extension RPC group をすでに expose しています。extension install / blocklist operation は現時点では daemon の HTTP/JSON beta transport で expose されています。`ateliad` の Rust RPC boundary は transport-neutral のままにしてあり、将来の proto/gRPC client path が同じ contract に接続できるようにしています。`WatchEvents` は現時点では cursor replay surface として実装されており、long-lived streaming transport は future transport refinement です。
