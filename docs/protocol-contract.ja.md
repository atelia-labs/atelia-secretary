...
`ListRepertoire` は [Agent Repertoire](https://github.com/atelia-labs/atelia/blob/main/docs/agent-repertoire.ja.md) の最初の beta protocol slice として予定されています。現在の context にある live tool surface の computed projection を返し、persisted store は返しません。

最初の beta server surface は意図的に小さく、現時点では built-in Secretary tool の `fs.read` と `secretary.echo` を stable tool definition として projection します。extension-backed の repertoire entry は次の slice です。
