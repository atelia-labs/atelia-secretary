...
この table は required service contract であり、現在の protobuf implementation だけの説明ではありません。現在の daemon は health、repository、job、policy、event replay、project status、tool-output settings、`RenderToolOutput`、extension RPC group をすでに expose しています。extension registry / blocklist operation は現時点では daemon の HTTP/JSON beta transport で expose されています。`ateliad` の Rust RPC boundary は transport-neutral のままにしてあり、将来の proto/gRPC client path が同じ contract に接続できるようにしています。`WatchEvents` は現時点では cursor replay surface として実装されており、long-lived streaming transport は future transport refinement です。
...
| `InstallExtension` | new extension manifest を install する |
| `UpdateExtension` | installed extension manifest を update する |
| `ExtensionStatus` | 1つの extension installation と blocklist state を inspect する |
| `ListExtensions` | installed extension status を list する |
| `RollbackExtension` | extension の previous version を restore する |
| `DisableExtension` | installed extension を disable する |
| `EnableExtension` | disabled extension を enable する |
| `RemoveExtension` | installed extension を remove する |
| `ApplyBlocklist` | blocklist entry を追加する |
| `ListBlocklist` | current blocklist を inspect する |

`ListRepertoire` は [Agent Repertoire](https://github.com/atelia-labs/atelia/blob/main/docs/agent-repertoire.ja.md) の最初の beta protocol slice です。現在の context にある live tool surface の computed projection を返し、persisted store は返しません。

最初の beta server surface は意図的に小さく、現時点ではこの beta slice で dispatch 可能な built-in Secretary tool だけ、すなわち `fs.read` と `secretary.echo` を stable tool definition として projection します。より広い built-in は将来の slice または runtime-backed slice に存在し得ますが、dispatch が存在するまでは `ListRepertoire` では claim しません。extension-backed の repertoire entry は次の slice です。

beta slice では extension management API は operator-facing です。
extension execution の RPC は beta では未対応です。endpoint が存在する場合でも、
invoke されたら unsupported-capability を返さなければなりません。
