...
最初の beta server surface は意図的に小さく、現時点では built-in Secretary tool の `fs.read`、`fs.list`、`fs.stat`、`fs.search`、`fs.diff`、`fs.write`、`fs.patch`、`fs.delete`、`fs.move`、`proc.exec`、`proc.run`、`secretary.echo` を stable tool definition として projection します。extension-backed の repertoire entry は次の slice です。
