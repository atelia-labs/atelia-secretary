...
Client UX、Hooks、AEP package、Surface Protocol の規範的仕様は Atelia 本体リポジトリに置きます。
...
- Claude Code は、`CLAUDE.md`、Skills、MCP、subagents、hooks、plugins、agent teams のような拡張層を持つ。Atelia では AEP package、host-rendered surface、Secretary / 専任エージェントの分業に接続できる
...
研究から仕様へ進めるときは、Atelia と Secretary の思想文書に照らして確認します。特に、Secretary の判断主体性、仕事場の成長、AX Feedback、tool output の token efficiency、package / hook が agency を奪わないことを確認します。
