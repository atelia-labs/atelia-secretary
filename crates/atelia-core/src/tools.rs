...
//! `fs.diff`, `fs.write`, `fs.patch`, `fs.delete`, and `fs.move` tools that implement
...
#[cfg(unix)]
fn acquire_ordered_write_locks(first_path: &Path, second_path: &Path) -> io::Result<Vec<File>> {
    ...
    if first_key <= second_key {
        Ok(vec![
            ...
            acquire_write_lock(second_path)?,
        ])
    } else {
        Ok(vec![
            acquire_write_lock(second_path)?,
...
impl crate::runtime::RuntimeTool for FsDeleteTool {
    fn tool_id(&self) -> &'static str {
        "fs.delete"
    }
    ...
    fn declared_effect(&self) -> &'static str {
        "delete one file within the registered repository scope"
    }

    fn args_summary(&self, request: &RuntimeJobRequest) -> String {
        let mut parts = vec![format!("path={}", request.resource_scope.value)];
        if self.allow_missing {
            parts.push("missing=ok".to_string());
        }
        parts.join(" ")
    }
    ...
    fn execute(&self, invocation: &ToolInvocation, request: &RuntimeJobRequest) -> ToolResult {
        ...
        let target = match resolve_mutation_target(&self.repository_root, &relative) {
                    ...
                    vec![
                        ToolResultField {
                            ...
                            value: StructuredValue::String(format!(
                                "delete skipped; {} was already missing",
                                request.resource_scope.value
                            )),
                        },
                        ToolResultField {
            ...
            Err(err) => {
                return failed_result(
                    invocation,
                    schema_ref,
                    "delete failed: path rejected".to_string(),
                    err.to_string(),
                );
            }
        };
        ...
        let metadata = match fs::symlink_metadata(path) {
            Ok(metadata) => metadata,
            Err(err) if err.kind() == io::ErrorKind::NotFound && self.allow_missing => {
                return make_tool_result(
                    invocation,
                    ToolResultStatus::Succeeded,
                    schema_ref,
            ...
            Err(err) => {
                return failed_result(
                    invocation,
                    schema_ref,
                    "delete failed: cannot read target metadata".to_string(),
                    format!("{}: {}", target.display_path(), err),
                );
            }
        };
        ...
        if !metadata.is_file() {
            return failed_result(
                invocation,
                schema_ref,
                "delete failed: target is not a file".to_string(),
                target.display_path().to_string(),
            );
        }
        ...
        if let Err(err) = delete_result {
            ...
            return failed_result(
                invocation,
                schema_ref,
                "delete failed: cannot remove file".to_string(),
                format!("{}: {}", target.display_path(), err),
            );
        }
...
impl FsMoveTool {
    pub fn new(repository_root: impl Into<PathBuf>, destination_path: impl Into<PathBuf>) -> Self {
        Self {
            repository_root: repository_root.into(),
...
impl crate::runtime::RuntimeTool for FsMoveTool {
    fn tool_id(&self) -> &'static str {
        "fs.move"
    }
    ...
    fn declared_effect(&self) -> &'static str {
        "move one file within the registered repository scope"
    }

    fn args_summary(&self, request: &RuntimeJobRequest) -> String {
        let mut parts = vec![
            format!("from={}", request.resource_scope.value),
            format!("to={}", self.destination_path.to_string_lossy()),
        ];
        if self.allow_overwrite {
            parts.push("overwrite=true".to_string());
        }
    ...
    fn resolved_paths(&self, request: &RuntimeJobRequest) -> Vec<ResolvedPath> {
        ...
        if let Ok(source) =
            resolve_mutation_target(&self.repository_root, &target_from_request(request))
        {
            paths.push(ResolvedPath {
                requested_path: request.resource_scope.value.clone(),
                resolved_path: source.path().to_string_lossy().to_string(),
                display_path: source.display_path().to_string(),
            });
        }
        if let Ok(destination) =
            resolve_mutation_target(&self.repository_root, &self.destination_path)
        {
            paths.push(ResolvedPath {
                requested_path: self.destination_path.to_string_lossy().to_string(),
                resolved_path: destination.path().to_string_lossy().to_string(),
                display_path: destination.display_path().to_string(),
            });
        }
        paths
    }
    ...
    fn execute(&self, invocation: &ToolInvocation, request: &RuntimeJobRequest) -> ToolResult {
            ...
            match resolve_mutation_target(&self.repository_root, &target_from_request(request)) {
                ...
                Err(err) => {
                    return failed_result(
                        invocation,
                        schema_ref,
                        "move failed: source path rejected".to_string(),
                        err.to_string(),
                    );
                }
            };
        let destination =
            match resolve_mutation_target(&self.repository_root, &self.destination_path) {
                Ok(target) => target,
                Err(err) => {
                    return failed_result(
                        invocation,
                        schema_ref,
                        "move failed: destination path rejected".to_string(),
                        err.to_string(),
                    );
                }
            };
        ...
        let source_metadata = match fs::symlink_metadata(source.path()) {
            Ok(metadata) => metadata,
            Err(err) => {
                return failed_result(
                    invocation,
                    schema_ref,
                    "move failed: cannot read source metadata".to_string(),
                    format!("{}: {}", source.display_path(), err),
                );
            }
        };
        if !source_metadata.is_file() {
            return failed_result(
                invocation,
                schema_ref,
                "move failed: source is not a file".to_string(),
                source.display_path().to_string(),
            );
        }
        ...
        let move_result: io::Result<bool> = {
            ...
            let destination_parent = destination.path().parent().ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidInput,
            ...
            match (
                ...
                (
                    ...
                    let locks = acquire_ordered_write_locks(source.path(), destination.path());
                    let source_parent_dir = open_parent_no_follow(source_parent);
...
mod tests {
    ...
    #[test]
    fn fs_delete_rejects_directories() {
        let env = TestEnv::new("delete-directory");
        env.create_dir("src");

        let tool = FsDeleteTool::new(&env.root);
        let invocation = fake_invocation(tool.tool_id());
        let request = request_with_mutation_path("src");
        let result = tool.execute(&invocation, &request);

        assert_eq!(ToolResultStatus::Failed, result.status);
        assert!(env.root.join("src").exists());
        let error = result.fields.iter().find(|f| f.key == "error").unwrap();
        assert!(string_value(&error.value).contains("src"));
        env.cleanup();
    }
    ...
    #[cfg(unix)]
    #[test]
    fn fs_delete_rejects_symlink_escape() {
        let env = TestEnv::new("delete-symlink-escape");
        let outside = unique_test_dir("delete-symlink-escape-outside");
        fs::write(outside.join("secret.txt"), "secret").unwrap();
        env.create_symlink("escape.txt", &outside.join("secret.txt"));

        let tool = FsDeleteTool::new(&env.root);
        let invocation = fake_invocation(tool.tool_id());
        let request = request_with_mutation_path("escape.txt");
        let result = tool.execute(&invocation, &request);
    ...
    fn fs_move_moves_file_within_scope() {
        ...
        env.create_file("notes.txt", "move me");
        env.create_dir("archive");

        let tool = FsMoveTool::new(&env.root, "archive/notes.txt");
        let invocation = fake_invocation(tool.tool_id());
        let request = request_with_mutation_path("notes.txt");
        let result = tool.execute(&invocation, &request);
    ...
    #[test]
    fn fs_move_rejects_overwrite_without_permission() {
        let env = TestEnv::new("move-overwrite-blocked");
        env.create_file("from.txt", "source");
