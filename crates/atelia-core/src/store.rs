...
pub trait SecretaryStore: Send + Sync + 'static {
    ...
    /// Append a terminal job event and persist the matching submit-job idempotency receipt.
    ...
    /// Look up the durable receipt recorded for a submit-job idempotency key.
    fn get_submit_job_idempotency(
        &self,
        idempotency_key: &str,
    ) -> StoreResult<Option<SubmitJobIdempotencyRecord>>;

    /// Store or verify a durable submit-job idempotency receipt.
    ...
    /// Append the terminal submit-job event together with the tool result,
...
pub struct SubmitJobIdempotencyRecord {
    /// Canonical signature used to detect whether a retry matches the original request.
    pub signature: String,
    /// Receipt captured after the job reaches a terminal successful state.
...
impl InMemoryStore {
    ...
    fn subscribe_repository_job_events(
        ...
        self.repository_event_broadcaster(repository_id).subscribe()
    }
    ...
    fn repository_event_broadcaster(
        ...
        let mut broadcasters = self
            ...
            .lock()
            .expect("repository event broadcaster lock poisoned");
        broadcasters
    ...
    fn publish_job_events(&self, inner: &InMemoryInner, events: &[JobEvent]) {
        for event in events {
            let event = event.clone();
            let _ = self.event_broadcaster.send(event.clone());
            if let Ok(Some(repository_id)) = event_repository_id(inner, &event) {
                let _ = self
    ...
    fn mutate_with_published_events<R>(
        ...
        *guard = draft;
        self.publish_job_events(&guard, &events);
        Ok(result)
    }
...
impl SecretaryStore for InMemoryStore {
    ...
    fn record_tool_result_audit_terminal_with_submit_job_idempotency(
        ...
        self.mutate(|inner| {
            ...
            ensure_event_sequence_capacity(inner, 3)?;

            let terminal_status = match &tool_result.status {
                crate::domain::ToolResultStatus::Succeeded => JobStatus::Succeeded,
                crate::domain::ToolResultStatus::Failed
                | crate::domain::ToolResultStatus::TimedOut => JobStatus::Failed,
                crate::domain::ToolResultStatus::Canceled => JobStatus::Canceled,
            };
            let mut final_record = record.clone();
            final_record
                .transition_status(terminal_status, terminal_event.created_at)
            ...
            validate_job_update_against(&record, &final_record, &terminal_event)?;
            validate_new_job_event_with_pending_records(
                inner,
                &terminal_event,
                Some(&final_record),
            ...
            record.latest_event_id = Some(audit_event.id.clone());
            inner.jobs.insert(record.id.clone(), record.clone());
            append_event_locked(inner, &mut terminal_event)?;
            final_record.latest_event_id = Some(terminal_event.id.clone());
            inner
                .jobs
                .insert(final_record.id.clone(), final_record.clone());

            if let Some((idempotency_key, mut idempotency_record)) = idempotency {
                let event_len = idempotency_record.receipt.events.len();
                if let Some(event) = idempotency_record
                    .receipt
                    .events
                    .get_mut(event_len.saturating_sub(3))
                {
                    *event = tool_result_event.clone();
                }
                if let Some(event) = idempotency_record
                    .receipt
                    .events
                    .get_mut(event_len.saturating_sub(2))
                {
                    *event = audit_event.clone();
                }
                if let Some(event) = idempotency_record.receipt.events.last_mut() {
                    *event = terminal_event.clone();
                }
                idempotency_record.receipt.job = final_record.clone();
                insert_submit_job_idempotency_locked(inner, idempotency_key, idempotency_record)?;
            }
...
fn is_schema_migration_lock(record: &SchemaMigrationRecord) -> bool {
    record.migration_name == MIGRATION_LOCK_RECORD_NAME && record.migration_lock.is_some()
}

fn schema_migration_lock_records(
    records: &HashMap<SchemaMigrationId, SchemaMigrationRecord>,
) -> impl Iterator<Item = &SchemaMigrationRecord> {
    records
        .values()
        .filter(|record| is_schema_migration_lock(record))
}

fn most_recent_schema_migration_lock_record(
    records: &HashMap<SchemaMigrationId, SchemaMigrationRecord>,
) -> Option<&SchemaMigrationRecord> {
    schema_migration_lock_records(records).max_by(|left, right| {
        left.created_at
...
fn validate_loaded_submit_job_receipt(
    ...
    validate_loaded_submit_job_receipt_event_chain(inner, receipt)?;
...
fn validate_loaded_submit_job_receipt_event_chain(
    inner: &InMemoryInner,
    receipt: &RuntimeJobReceipt,
) -> StoreResult<()> {
    let expected_events = expected_submit_job_receipt_events(inner, receipt)?;
    if expected_events.len() != receipt.events.len() {
        return Err(StoreError::InvalidRecord {
            collection: "submit_job_idempotency",
            reason: format!(
                "receipt.events must contain the complete job event chain for job {}",
                id_debug(&receipt.job.id)
            ),
        });
    }

    for (index, (expected_event, receipt_event)) in
        expected_events.iter().zip(&receipt.events).enumerate()
    {
        if *expected_event != receipt_event {
            return Err(StoreError::InvalidReference {
                collection: "submit_job_idempotency",
                reason: format!(
                    "receipt.events[{index}] does not match the stored job event chain for job {}",
                    id_debug(&receipt.job.id)
                ),
            });
        }
...
fn expected_submit_job_receipt_events<'a>(
    ...
    let latest_event_id =
        receipt
            ...
            .ok_or_else(|| StoreError::InvalidRecord {
                collection: "submit_job_idempotency",
                reason: format!(
                    "receipt.job.latest_event_id is missing for job {}",
                    id_debug(&receipt.job.id)
                ),
            })?;

    let mut expected_events = Vec::new();
    let mut found_latest_event = false;
    for event_id in inner.job_events_by_sequence.values() {
        let event = inner
            .job_events_by_id
            .get(event_id)
            .ok_or_else(|| StoreError::Conflict {
        ...
        if submit_job_receipt_event_belongs_to_job(inner, &receipt.job.id, event)? {
            if found_latest_event {
                return Err(StoreError::InvalidRecord {
                    collection: "submit_job_idempotency",
                    reason: format!(
                        "receipt.job.latest_event_id {} is not the end of the current job event chain for job {}",
                        id_debug(latest_event_id),
                        id_debug(&receipt.job.id)
                    ),
                });
    ...
    if !found_latest_event {
        return Err(StoreError::InvalidReference {
            collection: "submit_job_idempotency",
            reason: format!(
                "receipt.job.latest_event_id {} was not found in the stored job event chain for job {}",
                id_debug(latest_event_id),
                id_debug(&receipt.job.id)
            ),
        });
...
fn active_migration_lock<'a>(
    inner: &'a InMemoryInner,
    at: &LedgerTimestamp,
) -> Option<&'a SchemaMigrationRecord> {
    schema_migration_lock_records(&inner.schema_migrations)
        .filter(|record| {
            record
                .migration_lock
                .as_ref()
                .is_some_and(|lock| lock.is_active(at))
        })
        .max_by(|left, right| {
            left.created_at
                .cmp(&right.created_at)
...
fn schema_migration_lock_conflict_reason(active_lock: &SchemaMigrationRecord) -> String {
    format!(
        "active migration lock {} already held by {}",
        id_debug(&active_lock.id),
        active_lock
            .migration_lock
            .as_ref()
            .map_or("<unknown>", |migration_lock| migration_lock
                .leader_id
                .as_str())
    )
}

fn expire_schema_migration_locks(
    inner: &mut InMemoryInner,
    now: &LedgerTimestamp,
) -> StoreResult<()> {
...
fn validate_schema_migration_record(record: &SchemaMigrationRecord) -> StoreResult<()> {
    ...
    if record.migration_name == MIGRATION_LOCK_RECORD_NAME {
        ...
        if record.created_at != lock.locked_at {
            return Err(StoreError::InvalidRecord {
                collection: "schema_migrations",
    ...
    if record.migration_lock.is_some() {
        return Err(StoreError::InvalidRecord {
            collection: "schema_migrations",
            reason: "non-lock migration records cannot include migration_lock metadata".to_string(),
        });
    }
...
mod tests {
    ...
    use crate::domain::{
        ...
        JobEventKind, JobKind, LedgerTimestamp, LockOwner, LockedScope, MigrationLockStatus,
    ...
    #[test]
    fn schema_migration_records_reject_concurrent_active_lock() {
        let store = InMemoryStore::new();
        let active_lock = store
            .acquire_schema_migration_lock("daemon-a".to_string(), true, 5_000, timestamp(10))
            .unwrap();
        let migration =
            SchemaMigrationRecord::new_migration_lock("daemon-b", timestamp(10), false, 5_000)
                .unwrap();
        let active_leader_id = active_lock
            .migration_lock
            .as_ref()
            .expect("acquired lock must include metadata")
            .leader_id
            .clone();

        match store.create_schema_migration(migration) {
            Err(StoreError::Conflict { collection, reason }) => {
                ...
                assert!(reason.contains(&active_leader_id));
            }
    ...
    fn schema_migration_records_reject_released_lock_created_at_mismatch() {
        ...
        let mut migration =
            SchemaMigrationRecord::new_migration_lock("daemon-a", timestamp(10), true, 5_000)
                .unwrap();
        migration.created_at = timestamp(9);
        migration
            .migration_lock
            .as_mut()
            .expect("lock metadata must be present")
            .status = MigrationLockStatus::Released;
        migration
            .migration_lock
            .as_mut()
            .expect("lock metadata must be present")
    ...
    #[test]
    fn schema_migration_records_reject_expired_lock_created_at_mismatch() {
        let store = InMemoryStore::new();
        let mut migration =
            SchemaMigrationRecord::new_migration_lock("daemon-a", timestamp(10), true, 5_000)
                .unwrap();
        ...
        migration
            ...
            .as_mut()
            .expect("lock metadata must be present")
            .status = MigrationLockStatus::Expired;
        migration
            .migration_lock
            .as_mut()
            .expect("lock metadata must be present")
    ...
    #[test]
    fn schema_migration_lock_acquire_conflict_and_release() {
        let store = InMemoryStore::new();
        let first = store
            .acquire_schema_migration_lock("daemon-a".to_string(), true, 5_000, timestamp(10))
            .unwrap();
        ...
        assert!(matches!(
            store
                .acquire_schema_migration_lock("daemon-b".to_string(), false, 10_000, timestamp(10))
                .unwrap_err(),
            StoreError::Conflict {
                collection: "schema_migrations",
                ..
        ...
        let first_lock = first
            ...
            .expect("acquired lock must include lock metadata")
            .clone();
        let current = store.get_schema_migration_lock(timestamp(10)).unwrap();
        assert_eq!(current.as_ref().map(|record| &record.id), Some(&first.id));
        assert_eq!(current.unwrap().migration_lock.unwrap(), first_lock);

        assert!(matches!(
            store.release_schema_migration_lock(&first.id, "daemon-b", timestamp(20)),
            Err(StoreError::Conflict {
                collection: "schema_migrations",
                ..
            })
        ...
        assert_eq!(lock.released_at, Some(timestamp(20)));
        assert_eq!(released.updated_at, timestamp(20));

        let release_again = store
            .release_schema_migration_lock(&first.id, "daemon-a", timestamp(30))
            .unwrap();
        let lock = release_again
            .migration_lock
    ...
    #[test]
    fn schema_migration_lock_expires_and_can_be_reacquired() {
        let store = InMemoryStore::new();
        let first = store
            .acquire_schema_migration_lock("daemon-a".to_string(), true, 1, timestamp(1))
            .unwrap();
        let first_lock = first
            .migration_lock
            .expect("acquired lock must include lock metadata");
        let expired_after = first_lock.expires_at();
        assert_eq!(expired_after, timestamp(2));

        let second = store
            .acquire_schema_migration_lock("daemon-b".to_string(), false, 1_000, expired_after)
            .unwrap();
        assert_ne!(first.id, second.id);
        let lock = second
            .migration_lock
            .expect("acquired lock must include lock metadata");
        assert_eq!(lock.status, MigrationLockStatus::Held);
        assert_eq!(lock.leader_id, "daemon-b");
    }

    #[test]
    fn schema_migration_lock_records_are_listed_and_read_by_id() {
        let store = InMemoryStore::new();
        let lock = store
            .acquire_schema_migration_lock("daemon-list".to_string(), true, 1_000, timestamp(10))
            .unwrap();

        assert_eq!(
            store.get_schema_migration_lock(timestamp(11)).unwrap(),
            Some(lock.clone())
        );
        assert_eq!(store.get_schema_migration(&lock.id).unwrap(), lock);
        assert_eq!(store.list_schema_migrations().unwrap().len(), 1);
    }
    ...
    #[test]
    fn durable_store_rejects_submit_job_idempotency_with_incomplete_receipt_events() {
        let storage_dir = durable_storage_dir("idempotency-event-chain");
        let snapshot_path = storage_dir.join(DURABLE_STORE_FILE_NAME);
        let store = InMemoryStore::with_durable_storage_dir(&storage_dir).unwrap();
        let repository = repository_record();
        store.create_repository(repository.clone()).unwrap();
    ...
    #[test]
    fn durable_store_rejects_submit_job_idempotency_with_later_same_job_event() {
        let storage_dir = durable_storage_dir("idempotency-later-same-job-event");
        let snapshot_path = storage_dir.join(DURABLE_STORE_FILE_NAME);
        let store = InMemoryStore::with_durable_storage_dir(&storage_dir).unwrap();
        let repository = repository_record();
        store.create_repository(repository.clone()).unwrap();
        ...
        let mut invalid_inner = store.lock().unwrap().clone();
        let latest_event = receipt
            .events
            .last()
            .expect("receipt should contain events");
        let mut later_event = latest_event.clone();
        later_event.id = JobEventId::new();
        later_event.sequence_number = latest_event.sequence_number + 1;
        later_event.created_at = LedgerTimestamp::now();
        invalid_inner
            .job_events_by_id
            .insert(later_event.id.clone(), later_event.clone());
        invalid_inner
            .job_events_by_sequence
            .insert(later_event.sequence_number, later_event.id.clone());
        invalid_inner.next_event_sequence = later_event.sequence_number;
