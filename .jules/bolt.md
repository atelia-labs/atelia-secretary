...
## 2026-05-16 - Avoid full materialization during JobQuery pagination
**Learning:** In the core store `query_jobs`, pulling all filtered jobs into memory using `.collect::<Vec<_>>()` and then sorting using `.sort_by()` defeated the purpose of pagination. If there are tens of thousands of jobs, retrieving page 1 size 10 still eagerly allocates and sorts references for all 10,000 matches before retaining the requested page.
